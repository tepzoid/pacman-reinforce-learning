...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeActionFromQValues(self, state):
        ...
        if len(legal_acts) == 0:
            return None
        # Get the best action--the action with the highest Q-value
        else:
            max_val = float('-inf')
            candidate_acts = []
            # best_action = None
            ...
            # Return an action at random--tie-breaking
            return random.choice(candidate_acts)
    ...
    def update(self, state, action, nextState, reward):
        ...
        """
        "*** YOUR CODE HERE ***"
        cur_val = self.qvalues[(state, action)]

        new_estimate = reward + self.discount * self.computeValueFromQValues(nextState)

        self.qvalues[(state, action)] = (1 - self.alpha) * cur_val + self.alpha * new_estimate
